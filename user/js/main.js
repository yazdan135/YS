...
function manage_cart(pid, type) {
    var qty;
    if (type == 'update') {
        qty = jQuery("#" + pid + "qty").val(); // Using pid which corresponds to $key
    } else {
        qty = jQuery("#qty").val(); // For adding new products, you can adjust this
    }
    jQuery.ajax({
        url: 'manage_cart.php',
        type: 'post',
        data: { pid: pid, qty: qty, type: type }, // Send as an object
        success: function (result) {
            if (type == 'update' || type == 'remove') {
                window.location.href = 'cart.php'; // Reload the cart page after action
            }
            jQuery('.htc__qua').html(result); // Update any cart quantity display if needed
        }
    });
}
